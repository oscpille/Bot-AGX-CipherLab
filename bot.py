...
import time
import sys
import textwrap # <-- NUEVA LIBRERÍA para cortar texto inteligente
...
def inyectar_datos_en_tabla(diccionario_datos, es_ubicacion=False, es_flujo_volumen=False):
    ...
    # Pre-calcular si tenemos ambos datos para la regla de espaciado
    ...
    for nombre_logico, config in diccionario_datos.items():
        ...
        if es_ubicacion:
            # --- REGLA 2: ESPACIADO DE UBICACIÓN Y MARBETE ---
            ...
            # --- REGLA 3: FILTRO ANTI-DECIMALES EN VOLUMEN ---
            if es_flujo_volumen and config['tipo'] == 'real':
                print(f"   [!] Omitiendo '{config['nombre_pantalla']}' (Decimal) porque estamos en el Form 7 (Volumen).")
                continue # Salta este dato y no gasta el renglón
...
try:
    ...
    # Parche: Reemplaza posibles ';' por ':' para evitar errores de llenado
...
try:
    ...
    for i, linea in enumerate(lineas_cliente):
        if i > 2: break # Max 3 líneas permitidas (Filas 3, 4, 5)
        pyautogui.click(columnas["prompt"], filas_y[2 + i])
        time.sleep(0.1)
        pyautogui.write(linea, interval=0.05)
